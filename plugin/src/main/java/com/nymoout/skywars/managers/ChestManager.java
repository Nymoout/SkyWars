...
public class ChestManager {
	...
	public void load(Map<Integer, Inventory> itemList, String fileName) {
		itemList.clear();
		File chestFile = new File(SkyWars.get().getDataFolder(), fileName);
					...
					if (item != null && !item.getType().equals(Material.AIR)) {
						if (chest instanceof Chest) {
							if (random.nextInt(100) + 1 <= chance) {
								...
								added++;
								if (added >= inventory.getSize() - 1 || added >= SkyWars.getConfigManager().getMaxChest()) {
									break;
								}
							}
						}
						if (chest instanceof DoubleChest) {
							if (random.nextInt(100) + 1 <= chance) {
								...
								added++;
								if (added >= inventory.getSize() - 1 || added >= SkyWars.getConfigManager().getMaxDoubleChest()) {
									break;
								}
							}
						}
