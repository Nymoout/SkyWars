						...
						for (Player player: gMap.getAlivePlayers()) {
							if (player != null) {
								...
								for (int i = 0; i < per5Tick; i++) {
									UUID uuid = SkyWars.getNMS().spawnFallingBlock(loc.add(ThreadLocalRandom.current().nextDouble(-3, 3), ThreadLocalRandom.current().nextDouble(10, 20), ThreadLocalRandom.current().nextDouble(-3, 3)), Material.ANVIL, true).getUniqueId();
