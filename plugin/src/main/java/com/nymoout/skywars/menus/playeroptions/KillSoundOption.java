...
import com.nymoout.skywars.menus.playeroptions.PlayerOption;
...
public class KillSoundOption extends PlayerOption {
    ...
    private static void saveKillFile(String filename) {
		SkyWars.get().saveResource(filename, false);
		File sf = new File(SkyWars.get().getDataFolder(), filename);
		if (sf.exists()) {
			boolean result = sf.renameTo(new File(SkyWars.get().getDataFolder(), "killsounds.yml"));
			if (!result) {
				SkyWars.get().getLogger().info("Failed to rename Killsounds File");
			}
		}
    }
    ...
    public static void loadPlayerOptions() {
        ...
        if (!soundFile.exists()) {
			...
			} else if (SkyWars.getNMS().getVersion() < 13 && SkyWars.getNMS().getVersion() > 8) {
            ...
            if (storage.getConfigurationSection("sounds") != null) {
            	for (String key: storage.getConfigurationSection("sounds").getKeys(false)) {
                	...
                	if (mat != null) {
                    	...
                    	SkyWars.get().getServer().getLogger().info(material + " is not a valid Material in killsounds.yml");
                    }
            	}
            }
        }
	...
	public void playSound(Location loc) {
		if (SkyWars.get().getConfig().getBoolean("soundsEnabled", true)) {
			SkyWars.getNMS().playGameSound(loc, sound, volume, pitch, customSound);
		}
	}
