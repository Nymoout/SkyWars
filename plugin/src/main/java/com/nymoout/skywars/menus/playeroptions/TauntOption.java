...
public class TauntOption extends PlayerOption {
    ...
    private static void saveTauntFile(String filename) {
        ...
        if (sf.exists()) {
            boolean result = sf.renameTo(new File(SkyWars.get().getDataFolder(), "taunts.yml"));
            if (!result) {
                SkyWars.get().getLogger().info("Failed to rename Taunts File");
            }
        }
    }
    ...
    public void performTaunt(Player player) {
        if (!this.getKey().equalsIgnoreCase("none")) {
            SkyWars.getNMS().playGameSound(player.getLocation(), sound, volume, pitch, useCustomSound);
