...
public class WinSoundOption extends PlayerOption {
    ...
    private static void saveWinFile(String filename) {
        ...
        if (sf.exists()) {
            boolean result = sf.renameTo(new File(SkyWars.get().getDataFolder(), "winsounds.yml"));
            if (!result) {
                SkyWars.get().getLogger().info("Failed to rename Winsounds File");
            }
        }
    }
            ...
            if (storage.getConfigurationSection("sounds") != null) {
                for (String key : storage.getConfigurationSection("sounds").getKeys(false)) {
                    ...
                    if (mat != null) {
                        ...
                        SkyWars.get().getServer().getLogger().info(material + " is not a valid Material in winsounds.yml");
                    }
                }
            }
        }
