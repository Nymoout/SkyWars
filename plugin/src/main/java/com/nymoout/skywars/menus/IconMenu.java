...
public class IconMenu {
    ...
    private void addPrevItem(Inventory inventory) {
		inventory.setItem(inventory.getSize() - 9, SkyWars.getItemsManager().getItem("prevPageItem"));
	}

	private void addNextItem(Inventory inventory) {
		inventory.setItem(inventory.getSize() - 1, SkyWars.getItemsManager().getItem("nextPageItem"));
	}

	private void addExitItem(Inventory inventory) {
		inventory.setItem(inventory.getSize() - 5, SkyWars.getItemsManager().getItem("exitMenuItem"));
	}
	
    public void update() {
    	if (SkyWars.get().isEnabled()) {
        	Bukkit.getScheduler().scheduleSyncDelayedTask(SkyWars.get(), update);
    	}
    }
	...
	public void onInventoryClick(InventoryClickEvent event) {       
        ...
        if (event.getInventory().getItem(slot) != null && !event.getInventory().getItem(slot).getType().equals(Material.AIR)) {
            name = SkyWars.getNMS().getItemName(event.getCurrentItem());
        }     

        if (!name.equalsIgnoreCase("uselessName")) {
            if (name.equalsIgnoreCase(SkyWars.getNMS().getItemName(SkyWars.getItemsManager().getItem("prevPageItem")))) {
            	openInventory((Player) event.getWhoClicked(), index - 1);
            	return;
            }
            if (name.equalsIgnoreCase(SkyWars.getNMS().getItemName(SkyWars.getItemsManager().getItem("nextPageItem"))) && index + 1 < invs.size()) {
            	openInventory((Player) event.getWhoClicked(), index + 1);
            	return;
            }
