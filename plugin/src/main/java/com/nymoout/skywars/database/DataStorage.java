...
public class DataStorage {
    ...
    public void saveStats(final PlayerStat pData) {
        ...
        if (!sqlEnabled) {
            ...
            Database database = SkyWars.getDatabase();
            ...
            public void run() {
                ...
                if (sqlEnabled) {
                    ...
                    try {
                        ...
                        if (!playerDataDirectory.exists() && !playerDataDirectory.mkdirs()) {
                            SkyWars.get().getLogger().info("Encountered an error while creating data directory!");
                        }
    ...
    public void removePlayerData(String uuid) {
        ...
        if (!sqlEnabled) {
            try {
                ...
                if (!playerDataDirectory.exists() && !playerDataDirectory.mkdirs()) {
                    SkyWars.get().getLogger().info("Failed to create data directory");
                }

                File playerFile = new File(playerDataDirectory, uuid + ".yml");
                if (!playerFile.exists() && !playerFile.createNewFile()) {
                    SkyWars.get().getLogger().info("Failed to create playerfile for: " + uuid);
                }
                playerFile.delete();
            } catch (IOException ioException) {
