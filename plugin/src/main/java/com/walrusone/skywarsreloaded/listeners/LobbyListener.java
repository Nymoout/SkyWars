...
public class LobbyListener implements Listener {
    ...
    public void onEntityDamage(final EntityDamageByEntityEvent e) {
        if (SkyWarsReloaded.getConfigManager().protectLobby() && Util.get().isSpawnWorld(e.getEntity().getWorld())) {
            e.setCancelled(true);
    ...
    public void onInventoryClick(final InventoryClickEvent e) {
        ...
        if (SkyWarsReloaded.getConfigManager().protectLobby() && Util.get().isSpawnWorld(player.getWorld())) {
            if (!player.hasPermission("sw.alterlobby") && !SkyWarsReloaded.getIconMenuController().has(player)) {
                e.setCancelled(true);
            }
        }
    }
    ...
    public void onPlayerDropItem(final PlayerDropItemEvent e) {
        if (SkyWarsReloaded.getConfigManager().protectLobby() && Util.get().isSpawnWorld(e.getPlayer().getWorld())) {
                    ...
                    if (b.getType() == Material.WALL_SIGN || b.getType() == SkyWarsReloaded.getNMS().getMaterial("SIGN_POST").getType()) {
                        ...
                        if (SkyWarsReloaded.get().getUseable().contains(lines[1].toUpperCase())) {
                            ...
                            if (Util.get().isInteger(lines[2])) {
                                if (Integer.valueOf(lines[2]) <= SkyWarsReloaded.getConfigManager().getLeaderSize()) {
        ...
        if (Util.get().isSpawnWorld(e.getPlayer().getWorld())) {
            ...
            if (gMap == null) {
                if (e.getAction() == Action.PHYSICAL && (e.getClickedBlock().getType() == SkyWarsReloaded.getNMS().getMaterial("STONE_PLATE").getType() ||
                    ...
                    if (SkyWarsReloaded.getConfigManager().pressurePlateJoin()) {
                        Location spawn = SkyWarsReloaded.getConfigManager().getSpawn();
                        if (spawn != null) {
