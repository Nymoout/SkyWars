...
public class OptionSelectionMenu {
    ...
    public OptionSelectionMenu(final Player player, PlayerOptions p, boolean commandOpen) {
        ...
        for (PlayerOption option : availableItems) {
            ...
            ItemStack item = SkyWarsReloaded.getItemsManager().getItem("nopermission");
        ...
        SkyWarsReloaded.getIconMenuController().create(player, invs, event -> {
            ...
            if (SkyWarsReloaded.getConfigManager().economyEnabled() && !player.hasPermission(option.getPermission())) {
