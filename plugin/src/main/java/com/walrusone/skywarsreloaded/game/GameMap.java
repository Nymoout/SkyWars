...
public class GameMap {
    ...
    public GameMap(final String name) {
        ...
        timer = SkyWarsReloaded.getConfigManager().getWaitTimer();
        ...
        if (legacy) {
            ...
            if (loaded) {
                ...
                saveArenaData();
                SkyWarsReloaded.getWorldManager().deleteWorld(name);
            }
        }
        if (registered) {
            registerMap();
        ...
        if (SkyWarsReloaded.getConfigManager().joinMenuEnabled()) {
    ...
    private boolean isOutsideBorder(Player p) {
        ...
        double size = SkyWarsReloaded.getConfigManager().getBorderSize() / 2;
    ...
    public void update() {
        ...
        if (SkyWarsReloaded.getIconMenuController().has("joinsinglemenu") && teamSize == 1) {
            SkyWarsReloaded.getIconMenuController().getMenu("joinsinglemenu").update();
        }
        if (SkyWarsReloaded.getIconMenuController().has("jointeammenu") && teamSize > 1) {
            SkyWarsReloaded.getIconMenuController().getMenu("jointeammenu").update();
            if (matchState == MatchState.WAITINGSTART && SkyWarsReloaded.getIconMenuController().has(name + "teamselect")) {
                SkyWarsReloaded.getIconMenuController().getMenu(name + "teamselect").update();
            }
        }
    }
    ...
    public boolean removeMap() {
        ...
        SkyWarsReloaded.getWorldManager().deleteWorld(target);
    ...
    public static World createNewMap(String mapName, World.Environment environment) {
        World newWorld = SkyWarsReloaded.getWorldManager().createEmptyWorld(mapName, environment);
        if (newWorld == null) {
            return null;
        }
        addMap(mapName);
    ...
    public void stopGameInProgress() {
        ...
        SkyWarsReloaded.getWorldManager().deleteWorld(this.getName());
    }
    ...
    private static boolean loadWorldForScanning(String name) {
        ...
        WorldManager wm = SkyWarsReloaded.getWorldManager();
        ...
        boolean loaded = SkyWarsReloaded.getWorldManager().loadWorld(name, World.Environment.NORMAL);
        if (!loaded) {
    ...
    private void loadMap() {
        ...
        if (mapExists) {
            SkyWarsReloaded.getWorldManager().deleteWorld(mapName);
        }

        wm.copyWorld(source, target);

        boolean loaded = SkyWarsReloaded.getWorldManager().loadWorld(mapName, World.Environment.valueOf(environment));

        if (loaded) {
            World world = SkyWarsReloaded.get().getServer().getWorld(mapName);
            world.setSpawnLocation(2000, 0, 2000);
            if (SkyWarsReloaded.getConfigManager().borderEnabled()) {
                WorldBorder wb = world.getWorldBorder();
                wb.setCenter(teamCards.get(0).getSpawn().getX(), teamCards.get(0).getSpawn().getZ());
                wb.setSize(SkyWarsReloaded.getConfigManager().getBorderSize());
            }
    ...
    private void ChunkIterator(boolean message, Player sender) {
        ...
        int mapSize = SkyWarsReloaded.getConfigManager().getMaxMapSize();
        int max1 = mapSize / 2;
        int min1 = -mapSize / 2;
    ...
    private static boolean loadWorld(String worldName, GameMap gMap) {
        ...
        if (mapExists) {
            SkyWarsReloaded.getWorldManager().deleteWorld(worldName);
        }
        SkyWarsReloaded.getWorldManager().copyWorld(source, target);
        return SkyWarsReloaded.getWorldManager().loadWorld(worldName, World.Environment.valueOf(gMap.environment));
    }
    ...
    public void updateArenaManager() {
        if (SkyWarsReloaded.getIconMenuController().has(arenakey)) {
            SkyWarsReloaded.getIconMenuController().getMenu(arenakey).update();
        }
    }

    public static void openArenasManager(Player player) {
        if (player.hasPermission("sw.arenas")) {
            SkyWarsReloaded.getIconMenuController().show(player, "arenasmenu");
        }
    }

    public static void updateArenasManager() {
        if (SkyWarsReloaded.getIconMenuController().has("arenasmenu")) {
            SkyWarsReloaded.getIconMenuController().getMenu("arenasmenu").update();
        }
    }
    ...
    private void sendBungeeUpdate() {
        if (SkyWarsReloaded.getConfigManager().bungeeMode()) {
            ...
            if (player != null) {
                SkyWarsReloaded.get().sendSWRMessage(player, SkyWarsReloaded.getConfigManager().getBungeeLobby(), messages);
            }
        }
    }
    ...
    public void saveMap(Player mess) {
        ...
        Location respawn = SkyWarsReloaded.getConfigManager().getSpawn();
        ...
        if (editWorld != null) {
            ...
            SkyWarsReloaded.getWorldManager().deleteWorld(source);
            saveArenaData();
            inEditing = false;
            success = true;
        }
