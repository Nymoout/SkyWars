...
public class SWRSign {
    ...
    private static void updateBlock(Block block, String item) {
        block.setType(SkyWarsReloaded.getItemsManager().getItem(item).getType());
        if (SkyWarsReloaded.getNMS().getVersion() < 13) {
            if (SkyWarsReloaded.getItemsManager().getItem(item).getType().equals(Material.valueOf("WOOL")) || SkyWarsReloaded.getItemsManager().getItem(item).getType().equals(Material.valueOf("STAINED_GLASS")) || SkyWarsReloaded.getItemsManager().getItem(item).getType().equals(Material.valueOf("STAINED_CLAY"))) {
                SkyWarsReloaded.getNMS().setBlockWithColor(block.getWorld(), block.getX(), block.getY(), block.getZ(), SkyWarsReloaded.getItemsManager().getItem(item).getType(), SkyWarsReloaded.getItemsManager().getItem(item).getData().getData());
            }
        }
