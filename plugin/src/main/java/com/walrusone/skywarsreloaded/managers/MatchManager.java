...
public class MatchManager {
    ...
    public void start(final GameMap gameMap) {
        debug = SkyWarsReloaded.getConfigManager().debugEnabled();
        if (debug) {
        ...
        this.setWaitTime(SkyWarsReloaded.getConfigManager().getWaitTimer());
        this.setGameTime();
        ...
        if (pCard.getPlayer() != null && pCard.getTeamCard().getSpawn() != null && gameMap.getMatchState().equals(MatchState.WAITINGSTART)) {
            ...
            if (ps != null) {
                ...
                if (effect != null) {
                    ...
                    SkyWarsReloaded.getPlayerOptionsManager().addPlayer(player.getUniqueId(), effects);
                }
            }
            Util.get().clear(player);
            if (SkyWarsReloaded.getConfigManager().titlesEnabled()) {
            ...
            for (final Player p : gameMap.getAlivePlayers()) {
                if (!p.equals(player)) {
                    Util.get().playSound(p, p.getLocation(), SkyWarsReloaded.getConfigManager().getJoinSound(), 1, 1);
                }
            }
    ...
    private void preparePlayer(Player player, GameMap gameMap) {
        ...
        if (SkyWarsReloaded.getConfigManager().areKitsEnabled()) {
            ItemStack kitItem = SkyWarsReloaded.getItemsManager().getItem("kitvote");
            player.getInventory().setItem(SkyWarsReloaded.getConfigManager().getKitVotePos(), kitItem);
        }

        if (SkyWarsReloaded.getConfigManager().votingEnabled()) {
            ItemStack timeItem = SkyWarsReloaded.getItemsManager().getItem("votingItem");
            player.getInventory().setItem(SkyWarsReloaded.getConfigManager().getVotingPos(), timeItem);
        }

        ItemStack exitItem = SkyWarsReloaded.getItemsManager().getItem("exitGameItem");
        player.getInventory().setItem(SkyWarsReloaded.getConfigManager().getExitPos(), exitItem);
                ...
                if (gameMap.getFullTeams() >= gameMap.getMinTeams() || gameMap.getForceStart()) {
                    if (gameMap.getTimer() <= 0) {
                        ...
                        if (gameMap.getMatchState() != MatchState.ENDING) {
                            for (final Player player : gameMap.getAlivePlayers()) {
                                Util.get().playSound(player, player.getLocation(), SkyWarsReloaded.getConfigManager().getCountdownSound(), 1, 1F);
                            }
                        ...
                        if (gameMap.getTimer() <= 5 && gameMap.getMatchState() != MatchState.ENDING) {
                            for (final Player player : gameMap.getAlivePlayers()) {
                                ...
                                if (gameMap.getTimer() == 5) {
                                    Util.get().playSound(player, player.getLocation(), SkyWarsReloaded.getConfigManager().getCountdownSound(), 1, 0.5F);
                                } else if (gameMap.getTimer() == 4) {
                                    Util.get().playSound(player, player.getLocation(), SkyWarsReloaded.getConfigManager().getCountdownSound(), 1, 0.6F);
                                } else if (gameMap.getTimer() == 3) {
                                    Util.get().playSound(player, player.getLocation(), SkyWarsReloaded.getConfigManager().getCountdownSound(), 1, 0.7F);
                                } else if (gameMap.getTimer() == 2) {
                                    Util.get().playSound(player, player.getLocation(), SkyWarsReloaded.getConfigManager().getCountdownSound(), 1, 0.8F);
                                } else if (gameMap.getTimer() == 1) {
                                    Util.get().playSound(player, player.getLocation(), SkyWarsReloaded.getConfigManager().getCountdownSound(), 1, 0.9F);
                                }
                            }
                        }
    ...
    private void startMatch(final GameMap gameMap) {
        ...
        if (SkyWarsReloaded.getConfigManager().isTimeVoteEnabled()) {
            gameMap.getTimeOption().completeOption();
        }
        if (SkyWarsReloaded.getConfigManager().isWeatherVoteEnabled()) {
            gameMap.getWeatherOption().completeOption();
        }
        if (SkyWarsReloaded.getConfigManager().isModifierVoteEnabled()) {
            gameMap.getModifierOption().completeOption();
        }
        if (SkyWarsReloaded.getConfigManager().isHealthVoteEnabled()) {
    ...
    private void won(final GameMap gameMap, final TeamCard winners) {
        if (winners != null) {
            ...
            for (PlayerCard pCard : winners.getPlayerCards()) {
                ...
                if (win != null) {
                    ...
                    if (winnerData != null) {
                        ...
                        winnerData.setXp(winnerData.getXp() + (multiplier * SkyWarsReloaded.getConfigManager().getWinnerXP()));
                        if (gameMap.getTeamSize() == 1) {
                            winnerData.setElo(pCard.getPostElo());
                        }
                        if (SkyWarsReloaded.getConfigManager().economyEnabled()) {
                            VaultUtils.get().give(win, multiplier * SkyWarsReloaded.getConfigManager().getWinnerEco());
                        }
                        ...
                        if (SkyWarsReloaded.get().isEnabled()) {
                            new BukkitRunnable() {
                                @Override
                                public void run() {
                                    Util.get().sendActionBar(win, new Messaging.MessageFormatter().setVariable("xp", "" + multiplier * SkyWarsReloaded.getConfigManager().getWinnerXP()).format("game.win-actionbar"));
                                    Util.get().doCommands(SkyWarsReloaded.getConfigManager().getWinCommands(), win);
            ...
            String winName;
            if (SkyWarsReloaded.getConfigManager().usePlayerNames()) {
                    ...
                    public void run() {
                        for (PlayerCard pCard : winners.getPlayerCards()) {
                            ...
                            if (win != null) {
                                ...
                                if (SkyWarsReloaded.getConfigManager().fireworksEnabled()) {
                                    Util.get().fireworks(win, 5, SkyWarsReloaded.getConfigManager().getFireWorksPer5Tick());
                                }
        ...
        if (SkyWarsReloaded.get().isEnabled() && !gameMap.getMatchState().equals(MatchState.OFFLINE)) {
            ...
            new BukkitRunnable() {
                public void run() {
                    ...
                    new BukkitRunnable() {
                        public void run() {
                            if (SkyWarsReloaded.getConfigManager().bungeeMode()) {
                                Util.get().doCommands(SkyWarsReloaded.getConfigManager().getGameEndCommands(), null);
                            }
                            gameMap.refreshMap();
                            if (debug) {
                    ...
                    }.runTaskLater(SkyWarsReloaded.get(), (SkyWarsReloaded.getConfigManager().getTimeAfterMatch() * 20));
                }
            }.runTaskLater(SkyWarsReloaded.get(), (SkyWarsReloaded.getConfigManager().getTimeAfterMatch() * 20));
        }
    }
    ...
    public void playerLeave(final Player player, DamageCause dCause, final boolean leftGame, boolean sendMessages, boolean playerQuit) {
        SkyWarsReloaded.getPlayerOptionsManager().removePlayer(player.getUniqueId());
        ...
        if (gameMap.getMatchState() != MatchState.WAITINGSTART && gameMap.getMatchState() != MatchState.ENDING) {
            ...
            for (final Player p : gameMap.getAlivePlayers()) {
                Util.get().playSound(p, p.getLocation(), SkyWarsReloaded.getConfigManager().getLeaveSound(), 1, 1);
            }
    ...
    private void updatePlayerData(Player player, PlayerCard pCard, PlayerData playerData) {
        ...
        if (killerData != null) {
            ...
            killerData.setXp(killerData.getXp() + (multiplier * SkyWarsReloaded.getConfigManager().getKillerXP()));
        ...
        if (SkyWarsReloaded.getConfigManager().economyEnabled()) {
            VaultUtils.get().give(killer, multiplier * SkyWarsReloaded.getConfigManager().getKillerEco());
        }
        Util.get().sendActionBar(killer, new Messaging.MessageFormatter().setVariable("xp", "" + multiplier * SkyWarsReloaded.getConfigManager().getKillerXP()).format("game.kill-actionbar"));
        Util.get().doCommands(SkyWarsReloaded.getConfigManager().getKillCommands(), killer);
    }
