...
public class EffectSelectionMenu {
    ...
    public EffectSelectionMenu(final Player player) {
        List<ParticleItem> availableItems = SkyWars.getLM().getParticleItems();
        ...
        menuSize = rowCount;
        int level = com.nymoout.skywars.utilities.Util.get().getPlayerLevel(player);
        SkyWars.getIC().create(player, menuName, rowCount, new IconMenu.OptionClickEventHandler() {
            ...
            public void onOptionClick(IconMenu.OptionClickEvent event) {
                ...
                ParticleItem effect = SkyWars.getLM().getParticleByName(name);
                if (effect == null) {
                    return;
                }   
                ...
                if (SkyWars.getCfg().economyEnabled() && !player.hasPermission("sw.pareffect." + effect.getKey())) {
